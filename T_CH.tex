...
$\Ra \left| \{\AL t_1\} - \{\AL t_2\}  \right| < \frac{1}{x} \iff \left| \AL \underbrace{(t_1 - t_2)}_q - \underbrace{([\AL t_2] - [\AL t_1])}_p \right| \pe$

\begin{formbox}{}
\begin{teo}[Теорема Дирихле//(опять??)] $\AL \in \bb{R} \setminus \bb{Q} \Ra $ неравенство $\left|\AL - \frac{p}{q} \right| < \frac{1}{q^2}$ имеет бесконечное число решений в рациональных дробях.
\end{teo}
\end{formbox}
...
\begin{formbox}{}
\begin{teo} $\forall f(q) > 0\: \exists \AL:\:0 < \left|\AL - \frac{p}{q} \right| < f(q)$ имеет бесконечное число решений. (т.е. существует сколь угодно хорошее приближение рациональными дробями).
\end{teo}
\end{formbox}
$\pb \AL =\SL_{k=0}^\infty 10^{-n_k}, n_0 = 0, n_1 \le n_2 \le \cdots$\\
$\frac{p_m}{q_m} = \SL_{k=0}^m 10^{-n_k}, q_m = 10^{n_m}$\\
$\left|\AL - \frac{p_0}{q_0} \right| = \SL_{k=m_1}^\infty 10^{-n_k} < 2 \cdot 10^{-m_1} < f(q_0)$ (выбрали $m_1$ так чтобы выполнялось)\\
$\left|\AL - \frac{p_1}{q_1} \right| = \SL_{k=m_2}^\infty 10^{-n_k} < 2 \cdot 10^{-m_2} < f(q_1)$ и т.д. $\pe$
